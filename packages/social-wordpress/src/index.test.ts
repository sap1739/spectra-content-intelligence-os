...
import { parseWordPressCredential, WordPressCredentialError, WordPressPublisher } from './index';
...
describe('WordPressPublisher', () => {
  it('posts to the REST endpoint with Basic auth and maps a 201 to PUBLISHED', async () => {
    ...
    const publisher = new WordPressPublisher(CREDS, {
  ...
  it('rejects a non-http(s) site URL at construction', () => {
    expect(() => new WordPressPublisher({ ...CREDS, siteUrl: 'ftp://blog.example.com' })).toThrow(
      WordPressCredentialError,
    );
  });
});
